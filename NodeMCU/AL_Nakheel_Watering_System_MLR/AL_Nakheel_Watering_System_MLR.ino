...
bool connectionRequest = false, disconnectionRequest = false, Connected = false, tempSensorRunning = false, soilSensorRunning = false, palmAgeUpdated = false, systemStoped = false, sensorDamaged = false;
...
void WateringOperatin()
{
  ...
  if(millis() - startMillisWateringTime >= numOfSeconds) //Check the watering time
  {
    ...
    if(systemStatus!="Off" && !connectionRequest && !disconnectionRequest)
    {
      ...
      if(!sensorDamaged)
        waterAmount = ComputeWaterAmount(temperature, soilMoisture); //Compute water amount
